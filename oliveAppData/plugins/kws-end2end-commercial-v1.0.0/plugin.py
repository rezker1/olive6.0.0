...
class CustomPlugin(Plugin, RegionScorer):
    ...
    def __init__(self):
        self.task  = "KWS"
        self.label = "KWS End-to-End (Commercial)"
        self.description = "Perform KWS from ASR transcript using a wav2vec2 model. Segment audio with SAD DNN V7 multi-v1."
    ...
    def run_region_scoring(self, domain_id, audio, workspace, classes=None, opts=None):
        ...
        if not classes:
            self.escape_with_error("No keywords found, please provide at least one keyword.")
        else:
            logger.debug(print("List of keywords: "+" ".join(classes)))
            domain.keywords = classes
        ...
        if len(asr_out) > 0:
            ...
            kws_out = []
            begin_time, end_time, word, conf = [], [], [], []

            for key in asr_result.keys():
                item = asr_result[key][0]
                begin_time.append(item[0])
                end_time.append(item[1])
                word.append(item[2].lower())
                conf.append(item[3])

            for keyword in domain.keywords:
                len_keyword = len(keyword.split())
                for cnt in range(len(word)-len_keyword+1):
                    if " ".join(word[cnt:cnt+len_keyword]) == keyword.lower():
                        segment = (begin_time[cnt], end_time[cnt+len_keyword-1], " ".join(word[cnt:cnt+len_keyword]), conf[cnt])
                        kws_out.append(segment)

            kws_out.sort()
            kws_result = {index:[(np.float32(st), np.float32(et), wd, conf)] for index, [st, et, wd, conf] in enumerate(kws_out)}
            logger.debug("kws_result = {}".format(kws_result))
